...
def grism_model(xc_full=244, yc_full=1244, lam_spec=None, flux_spec=None, grow_factor=2, growx=2, growy=2, pad = 60, BEAMS=['A','B','C','D'], dydx=True):
...
class GrismModel():
    ...
    def get_corrected_wcs(self, verbose=True):
        ...
        for i in range(NOBJ):
            xi = (self.cat.x_pix[i]-self.pad/2.)/self.growx
            yi = (self.cat.y_pix[i]-self.pad/2.)/self.growy
            fp.write('%.2f %.2f\n' %(xi,  yi))
    ...
    def compute_object_model(self, id=328, lam_spec=None, flux_spec=None, BEAMS=['A','B','C','D'], verbose=False, normalize=True):
        ...
        t0 = time.time()
        orders, self.xi = unicorn.reduce.grism_model(xc, yc, lam_spec=lam_spec, flux_spec=flux_spec, BEAMS=BEAMS, grow_factor=self.grow_factor, growx=self.growx, growy=self.growy, pad=self.pad)
...
def interlace_aegis0():
    ...
    unicorn.reduce.prepare_blot_reference(REF_ROOT='AEGIS_F140W', filter='F140W', REFERENCE = '/3DHST/Photometry/Work/AEGIS/IMAGES/SMALL_PIXSCL/AEGIS_F140W_detect.fits', SEGM = '/3DHST/Photometry/Work/AEGIS/Sex/PSF_matched/F140W_seg.fits')
...
def interlace_cosmos0():
    ...
    os.chdir(unicorn.GRISM_HOME+'COSMOS/INTERLACE')
...
def interlace_cosmos2():
    ...
    os.chdir(unicorn.GRISM_HOME+'COSMOS/INTERLACE_v2.1')
...
def interlace_uds0():
    ...
    os.chdir(unicorn.GRISM_HOME+'UDS/INTERLACE')
