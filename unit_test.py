...
def run_wfc3(run_prep=True, run_interlace=True, run_redshifts=True):
    ...
    if run_prep:
        ...
        init.prep_direct_grism_pair(direct_asn='goodss-34-F140W_asn.fits', grism_asn='goodss-34-G141_asn.fits', radec='../Catalog/goodss_radec.dat', raw_path='../RAW/', mask_grow=8, scattered_light=False, final_scale=None, skip_direct=False, ACS=False)
        
        #### test
        # if True:
        #     import threedhst.prep_flt_astrodrizzle as init
        #     init.subtract_flt_background(root='goodss-34-F140W', scattered_light=False)
    ...
    if run_interlace:
        ...
        NGROW = 125
        unicorn.reduce.interlace_combine(root='goodss-34-F140W', view=False, use_error=True, make_undistorted=False, pad=60, NGROW=NGROW, ddx=0, ddy=0, growx=2, growy=2, auto_offsets=False, ref_exp=0)
        unicorn.reduce.interlace_combine(root='goodss-34-G141', view=False, use_error=True, make_undistorted=False, pad=60, NGROW=NGROW, ddx=0, ddy=0, growx=2, growy=2, auto_offsets=False, ref_exp=0)
        
        #### Interlaced reference
        adriz_blot = unicorn.reduce.adriz_blot_from_reference
        adriz_blot(pointing='goodss-34-F140W', pad=60, NGROW=NGROW, growx=2, growy=2, auto_offsets=False, ref_exp=0, ref_image='../Catalog/goodss_3dhst.v4.0.IR_cutout_sci.fits', ref_ext=0, ref_filter='F140W', seg_image='../Catalog/goodss_3dhst.v4.0.IR_cutout_seg.fits', cat_file='../Catalog/goodss_3dhst.v4.0.IR_cutout.cat')
        
        #### Make a model to 25 AB
        model = unicorn.reduce.process_GrismModel(root='goodss-34', grow_factor=2, growx=2, growy=2, MAG_LIMIT=25, REFINE_MAG_LIMIT=21, make_zeroth_model=False, use_segm=False, model_slope=0, model_list=None, direct='F140W', grism='G141', BEAMS=['A', 'B', 'C', 'D', 'E'])
    ...
    if run_redshifts:
        ...
        for id in ids:
            if os.path.exists('goodss-34_%05d.2D.fits' %(id)):
                continue
            #
            ...
            #
            ### Original fit
            gris = unicorn.interlace_fit.GrismSpectrumFit(root='goodss-34_%05d' %(id), FIGURE_FORMAT='png', verbose=True, lowz_thresh=0.4, fix_direct_thumbnail=True, RELEASE=False, OUTPUT_PATH='./', BASE_PATH='./', skip_photometric=False, p_flat=0.0001, use_mag_prior=True, dr_match=1.0)
            gris.fit_in_steps(dzfirst=0.003, zrfirst=(0.6, 3.3), dzsecond=0.0002, save=True, make_plot=True, skip_second=False, oned=False)
            ### New fit
            gris = test.SimultaneousFit(root='goodss-34_%05d' %(id), FIGURE_FORMAT='png', lowz_thresh=0.4)
...
def run_acs(run_prep=True, run_interlace=True, run_redshifts=True):
    ...
    if run_interlace:
        ...
        #### Interlaced reference
        adriz_blot = unicorn.reduce.adriz_blot_from_reference
        adriz_blot(pointing='goodss-34-03-F814W', ACS=True, pad=100, NGROW=0, growx=1, growy=1, auto_offsets=False, ref_exp=0, ref_image='goodss-34-03-F814W_drc_sci.fits', ref_ext=0, ref_filter='F140W', seg_image='Catalog/goodss_3dhst.v4.0.IR_cutout_seg.fits', cat_file='Catalog/goodss_3dhst.v4.0.IR_cutout.cat')
    ...
    if run_redshifts:
        ...
        for chip in ids.keys():
            ...
            for id in ids[chip]:
                ...
                #
                gris = unicorn.interlace_fit.GrismSpectrumFit(root='goodss-34-03-chip%d_%05d' %(chip, id), FIGURE_FORMAT='png', verbose=True, lowz_thresh=0.05, fix_direct_thumbnail=True, RELEASE=False, OUTPUT_PATH='./', BASE_PATH='./', skip_photometric=False, p_flat=0.0001, use_mag_prior=True, dr_match=1.0)
                #
                gris = unicorn.interlace_test.SimultaneousFit(root='goodss-34-03-chip%d_%05d' %(chip, id), FIGURE_FORMAT='png', verbose=True, lowz_thresh=0.05, fix_direct_thumbnail=True, RELEASE=False, OUTPUT_PATH='./', BASE_PATH='./', skip_photometric=False, p_flat=0.0001, use_mag_prior=True, dr_match=1.0)
                gris.new_fit_constrained(zrfirst=[0.05, 0.6], dzfirst=0.005, dzsecond=0.0005, make_plot=True, ignore_photometry=False, ignore_spectrum=False, refit_norm=False, get_tilt=True, faint_limit=24)
